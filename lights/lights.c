...
static int write_int(char const *path, int value)
{
	...
	if (fd >= 0) {
		...
		if (already_warned == 0) {
			SLOGE("write_int failed to open %s\n", path);
			already_warned = 1;
		}
		return -errno;
	}
...
static int set_light_leds_notification(struct light_device_t *dev, struct light_state_t const *state)
{
    return 0;
}
						...
						struct hw_device_t **device)
{
    ...
    if (strcmp(LIGHT_ID_BACKLIGHT, name) == 0)
        ...
        set_light = set_light_leds_notification;
